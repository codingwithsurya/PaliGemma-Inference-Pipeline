...
def add_image_tokens_to_prompt(prefix_prompt, bos_token, image_seq_len, image_token):
    # Adds image tokens and BOS token to the prompt
    return f"{image_token * image_seq_len}{bos_token}{prefix_prompt}\n"

def rescale(image: np.ndarray, scale: float, dtype: np.dtype = np.float32) -> np.ndarray:
    # Rescales image values by multiplying with a scale factor
...
def normalize(image: np.ndarray, mean: Union[float, Iterable[float]],
    ...
    # Normalizes image using provided mean and standard deviation
...
def process_images(images: List[Image.Image], size: Dict[str, int] = None,
    ...
    # Processes a list of images: resize, rescale, normalize, and transpose
...
class PaliGemmaProcessor:
    ...
    def __init__(self, tokenizer, num_image_tokens: int, image_size: int):
        # Initializes processor with tokenizer and adds special tokens
        super().__init__()
        ...
        # Add location and segment tokens
    ...
    def __call__(self, text: List[str], images: List[Image.Image],
        ...
        # Processes images and text for model input
